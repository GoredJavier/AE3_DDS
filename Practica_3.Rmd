...
```{r Grafico_Request_Time, cache=TRUE}
...
df_TimeVsRequest$Time1 <- as.POSIXct(df_TimeVsRequest$Time, format="%Y-%m-%d %H:%M:%S")  # Se convierte la columna time en formato de fecha y hora

# Crear la gráfica
ggplot(df_TimeVsRequest, aes(x = Time1, y = Request, color = RequestType )) +
  geom_line(Size=1) + # Grafica los valores en lineas
  scale_x_datetime(date_breaks = "1 hour", date_labels = "%Y-%m-%d %H:%M:%S") +  # Mostrar cada hora
  labs(title = "Peticiones Servidas por Tipo a lo Largo del Tiempo",
       x = "Hora",
       y = "Número de Peticiones") +
  theme_minimal() +  theme(axis.text.x = element_text(angle = 90, hjust = 1)) # Rotar etiquetas 90 grados
...
# Crear la gráfica
ggplot(df_TimeVsRequest, aes(x = Time1, y = Request, fill = RequestType )) +
  geom_bar(stat = "identity", position = "stack") +  # Crea barras apiladas
  scale_x_datetime(date_breaks = "1 hour", date_labels = "%Y-%m-%d %H:%M:%S") +  # Mostrar cada hora
  labs(title = "Peticiones Servidas por Tipo a lo Largo del Tiempo",
       x = "Hora",
       y = "Número de Peticiones") +
...
#print(df_TimeVsRequest$Request)
#df_TimeVsRequest
...
#df_TimeVsRequest$Request[df_TimeVsRequest$Request > 5]
#df_TimeVsRequest$RequestType[df_TimeVsRequest$Request > 5]

#df_TimeVsRequest$Time1 <- as.POSIXct(df_TimeVsRequest$Time, format="%Y-%m-%d %H:%M:%S")

#scale_x_date(date_breaks = "1 week", date_labels = "%b %d") + # Mostrar etiquetas cada semana
  
#scale_x_datetime(date_breaks = "1 hour", date_labels = "%H:%M") +  # Mostrar cada hora
